...
struct WelcomeView : View {
    ...
    private let sampleLevel = LevelMath.calculateLevel(fromExp: 780)
    private let sampleMistResult = MistCombatMath.applyPositivePost(
        currentHP: 75,
        maxHP: 100,
        currentHappyPostCount: 2,
        isMistCleanupPost: false,
        elapsedSeconds: 10,
        regenPerSecond: 1,
        damagePerPost: 10,
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            Text("Level sample: Lv \(sampleLevel) / 次まで \(LevelMath.expForNextLevel(sampleLevel))")
                .font(.subheadline)
            Text("Mist sample: HP \(sampleMistResult.nextHP), Count \(sampleMistResult.nextHappyPostCount)")
                .font(.subheadline)
