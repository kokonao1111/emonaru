...
final class QRCodeService {
    ...
    func generateQRCode(for userID: String) -> UIImage? {
        // 【暫定実装】カスタムURLスキームを使用
        // アプリがインストールされている端末でのみ動作します
        //
        // 【将来の改善案】
        // - App Store公開後: App Storeリンク + パラメータ
        // - 独自ドメイン取得後: Universal Links
        // - Branch.io: 自動振り分け（ビジネスメール必要）
        
        // カスタムURLスキーム（アプリ内専用）
        let urlString = "emotionapp://user/\(userID)"
        
        // TODO: App Store公開後、以下に切り替えてください
        // let appStoreID = "YOUR_APP_STORE_ID"
        // let urlString = "https://apps.apple.com/app/id\(appStoreID)?user=\(userID)"
        
        print("✅ QRコード用URL生成: \(urlString)")
